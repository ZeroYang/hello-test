...
void ClightningBoltSprite::setStartPoint(CCPoint startPoint){
    ...
    if(m_headSeg&&m_tailSeg)
    {
        ...
        for(int i=0;i<nseg;i++){
            ClineSeg*seg=m_segList[i];
            seg->startPoint=m_tailSeg->endPoint+rotateVector2(seg->startPoint-m_tailSeg->endPoint, cosA,sinA);
            seg->endPoint=m_tailSeg->endPoint+rotateVector2(seg->endPoint-m_tailSeg->endPoint, cosA,sinA);
            if(seg->branchID==1&&seg->father->branchID==0){//secondary branch root seg
                seg->branchEndPoint=m_tailSeg->endPoint+rotateVector2(seg->branchEndPoint-m_tailSeg->endPoint, cosA, sinA);
            }
        }
        //scale segList, use headSeg->startPoint as the center
        float dis_TailEndToStart=ccpDistance(m_tailSeg->endPoint, this->m_startPoint);
        float dis_TailEndToHeadStart=ccpDistance(m_tailSeg->endPoint, m_headSeg->startPoint);
        float scaleFactor=dis_TailEndToStart/dis_TailEndToHeadStart;
        for(int i=0;i<nseg;i++){
            ClineSeg*seg=m_segList[i];
            seg->startPoint=m_tailSeg->endPoint+ccpMult(seg->startPoint-m_tailSeg->endPoint, scaleFactor);
            seg->endPoint=m_tailSeg->endPoint+ccpMult(seg->endPoint-m_tailSeg->endPoint, scaleFactor);
            if(seg->branchID==1&&seg->father->branchID==0){//secondary branch root seg
                seg->branchEndPoint=m_tailSeg->endPoint+ccpMult(seg->branchEndPoint-m_tailSeg->endPoint, scaleFactor);
            }
        }
